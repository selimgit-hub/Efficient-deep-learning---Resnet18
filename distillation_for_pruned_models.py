...
student_model = Factorization.fact_models_dsc.ResNet18()

# Load pruned student model
checkpoint_student = torch.load('Lab1/Models/quantized_dsc_sp0.5.pth', map_location='cuda')
...
# Apply pruning mask to prevent pruned weights from being updated
for name, module in student_model.named_modules():
    if isinstance(module, nn.Linear):
        prune.identity(module, name='weight')  # Keeps track of pruned weights
        module.weight_orig.requires_grad = False  # Freeze pruned weights
...
# Define optimizer (update only non-zero weights)
optimizer = optim.SGD(filter(lambda p: p.requires_grad, student_model.parameters()), 
...
torch.save(state, 'Lab1/Models/cor_distilled_q_dsc_0.5.pth')
